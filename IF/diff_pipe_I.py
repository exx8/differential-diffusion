...
class DiffIFImg2ImgPipeline(DiffusionPipeline, LoraLoaderMixin):
    ...
    def __call__(
        ...
        map = torchvision.transforms.Resize(self.unet.sample_size, antialias=None)(map)
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                # diff diff begin of editing
                #Unfortunately diffusers doesn't support adding noise to multiple timesteps at once on this pipeline,
                # so we do it one by one, which is slower.
                original_picture_with_noise = self.prepare_intermediate_images(
                    image, t, batch_size, num_images_per_prompt, dtype, device, generator
                )
                if i == 0:
                    intermediate_images = original_picture_with_noise
                else:
                    mask = map > (i / len(timesteps))
                    # cast mask to the same type as intermediate_images etc
                    mask = mask.to(intermediate_images.dtype).to(intermediate_images.device)
                    mask = mask.unsqueeze(1)  # fit shape
                    # resize mask to latent size
                    intermediate_images = original_picture_with_noise * mask + intermediate_images * (1 - mask)
                # diff diff end of editing
