...
class KandinskyV22DiffImg2ImgPipeline(DiffusionPipeline):
    """
    ...
    def __call__(
        ...
        current_latents_encode = self.movq.encode(image)["latents"]
        current_latents_encode = current_latents_encode.repeat_interleave(num_images_per_prompt, dim=0)

        # prepartions
        original_with_noise = self.prepare_latents(
            current_latents_encode, timesteps, batch_size, num_images_per_prompt, image_embeds.dtype, device, generator
        )
        ...
        map = torchvision.transforms.Resize(tuple(s // self.movq_scale_factor for s in image.shape[2:]),antialias=None)(map)
